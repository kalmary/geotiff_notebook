...
def plot_mask(mask, ndvi, ax=None, path: Union[str, pth.Path] = None):
    ...
    display_ndvi = np.where(np.isnan(ndvi), np.nan, ndvi)
    _cmap = plt.get_cmap("RdYlGn").copy(); _cmap.set_bad("white")
    im = ax.imshow(display_ndvi, cmap=_cmap, vmin=-1, vmax=1)
...
def test_detector():
    ...
    display_ndvi = np.where(np.isnan(ndvi), np.nan, ndvi)
    plt.imshow(display_ndvi, cmap=_cmap, vmin=-1, vmax=1)
    overlay = np.where(mask, 1.0, np.nan)
    plt.imshow(overlay, cmap="Blues", alpha=0.6, vmin=0, vmax=1)
    plt.colorbar(label="NDVI")
    plt.subplot(1, 2, 2)
    plt.title("Augmented NDVI with Difference Mask Overlay")
    display_ndvi_aug = np.where(np.isnan(ndvi), np.nan, ndvi)
    plt.imshow(display_ndvi_aug, cmap=_cmap, vmin=-1, vmax=1)
